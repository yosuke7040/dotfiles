...
description: 日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。日本語で「PR description（/pr 経由か否かを問わず）」「設計書（ADR / 機能設計書 / Plan File）」「技術書の章・草稿・記事・解説文」「Linear への記載（issue / project / document / comment / status update など）」を書くとき、または推敲・リライトするときに使用する。読者向けに公開・共有する文書が対象。dev-journal / dev-archive のような作業ログ・進捗記録には適用しない（書き手＝読み手の記録は対象外）。
---
