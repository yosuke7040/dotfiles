...
- **Phase 7（PR 作成）はデフォルト実行**。`skip_pr = true` をユーザーが明示した場合、または feature ブランチに差分が存在しない場合のみスキップする。PR は **必ず `gh pr create --draft` で draft として作成** し、`claude/commands/pr.md` の執筆方針・セルフレビュー観点に従う。Phase 3 の PR1/PR2 で作成済みの PR と混同しない（Phase 7 は feature ブランチに対する最終 PR）
...
- `skip_pre_refactor` / `skip_refactor` / `skip_security` / `skip_pr`: 各フェーズをスキップするかのフラグ（ユーザーが明示した場合のみ true、デフォルト false）
- `feature_pr_url`: Phase 7 で作成した feature ブランチの draft PR URL
...
7. コメントは原則書かない。テスト名と arrange-act-assert 構造から意図が読めれば十分。WHY が非自明な場合（特殊な前提条件、回帰の出所、読み手が驚く挙動）のみ1行で残す。「test added for refactor safety」等の経緯コメントは書かない
```
...
5. コメントは原則書かない。元コードに無いコメントを増やさない。WHY が非自明な箇所（隠れた制約、微妙な不変条件、特定バグの回避策）のみ1行で残す。「extracted from ...」「refactored for ...」等の経緯コメントは書かない
```
...
4. 自信がない箇所はコード中にコメント（TODO/FIXME 含む）を残さず、最終報告に「保留事項」として明記すること。ユーザーには問い合わせず、オーケストレーター側で dev-journal に記録して次イテレーションに持ち越す
5. コメントは原則書かない。WHAT を説明するコメント、現在のタスク／設計判断／PR への言及、TODO/FIXME は書かない。WHY が非自明な場合（隠れた制約、微妙な不変条件、特定バグの回避策、読み手が驚く挙動）のみ1行で残す
```
...
3. 修正に自信がない場合は、修正せずに保留扱いとし、最終報告にその旨を残すこと。**コード中にコメント（TODO/FIXME 含む）を残してはならない**。ユーザーには問い合わせず、オーケストレーター側で dev-journal に記録して次イテレーションに持ち越す
4. テストやフォーマッタが存在する場合は実行して確認すること
5. レビュー指摘の "修正例" はあくまで参考。設計や周辺コードの整合性から外れている場合は、修正例に従わず自分の判断で根本原因に対処してよい（その旨をコミットメッセージか dev-journal に残す）
6. コメントは原則書かない。WHAT を説明するコメント、現在の finding／レビュー指摘への言及（「review fix for ...」「added per review」等）、TODO/FIXME は書かない。WHY が非自明な場合（隠れた制約、微妙な不変条件、特定バグの回避策、読み手が驚く挙動）のみ1行で残す
```
...
- 機能の振る舞いを変える提案・修正は禁止
- **「よりシンプルに実装できないか」を必ず観点に含める**（同じ振る舞いをより少ないコード・概念で表現できる箇所、標準ライブラリや既存ユーティリティへの置き換え、過剰な抽象の畳み込み等）
...
- コメントは原則書かない。WHY が非自明な場合のみ1行で残す。「cleaned up」「extracted from ...」「per refactor review」等の経緯コメントや TODO/FIXME は書かない
...
- コメントは原則書かない。WHY が非自明な箇所（防御の意図がコードから読み取れない場合等）のみ1行で残す。「security fix」「sanitize input」「per security review」等の自明な経緯コメントや TODO/FIXME は書かない
...
## Phase 7: Pull Request 作成（デフォルト有効・スキップ可）

**事前条件**: Phase 6 完了、または Phase 5/6 がスキップされたあと。feature ブランチ上に少なくとも 1 コミットの差分が存在する

**この Phase の絶対ルール**:
- `skip_pr = true` がユーザーから事前指定されている場合、または feature ブランチに差分がない場合はスキップして最終サマリーへ
- 作成する PR は **必ず draft**。`gh pr create --draft` を使用し、通常の PR として作成してはならない
- `claude/commands/pr.md` の「執筆方針」「生成時の考慮事項」「PR 作成後のセルフレビューと修正」を必ず参照し、それに従う
- description は WHY を厚く、HOW を薄く。冒頭は **背景 / 概要に相当する見出し** 直下の `-` 箇条書き 3 行で、1 行目は Linear issue リンク（`{linear_issue.url}`）
- Phase 3 の PR1（test）/ PR2（refactor）と混同しない。Phase 7 が扱うのは feature ブランチに対する最終 PR のみ
- 機密情報・内部 URL・API キー等が description に混入していないことを `gh pr edit` 前に必ず確認する

**成功条件**: feature ブランチの draft PR が作成され、セルフレビュー済みで Linear issue にリンクされている（`feature_pr_url` が確定し、`/dev-journal` に記録される）

### Step 0: 起動条件チェック

- `skip_pr = true` の場合: ジャーナルにスキップ理由を記録して最終サマリーへ
- `git diff {ベースブランチ}...HEAD` に差分がない場合: 差分なしを理由にスキップし、ジャーナルに記録して最終サマリーへ
- それ以外は Step 1 へ

### Step 1: ブランチを push

1. `git fetch origin` で最新化
2. 現在ブランチが feature ブランチであることを確認（main 上で実行されている場合は中断してユーザーに報告）
3. リモートに同名ブランチが既に存在するかを確認し:
   - 存在しない → `git push -u origin {feature_branch_name}`
   - 存在する & ローカルが先行 → 通常の `git push` で fast-forward push（force push は使わない）
   - 存在する & ローカルが乖離 → ユーザーに報告して停止（破壊的操作の独断回避）

### Step 2: PR description のドラフト生成

1. `claude/commands/pr.md` を Read し、執筆方針と生成時の考慮事項を確認する
2. PR テンプレートを以下の順で探索: `.github/PULL_REQUEST_TEMPLATE.md` → `PULL_REQUEST_TEMPLATE.md` → `docs/PULL_REQUEST_TEMPLATE.md`
3. description の素材として以下を活用する（**そのまま貼り付けず、WHY を組み立てる材料として使う**）:
   - `linear_issue.identifier` / `linear_issue.url` / `linear_issue.title`
   - `requirements_summary`（WHY と受け入れ条件の出所）
   - `design_summary`（採用した設計と代替案の出所）
   - `directives`（オシレーション収束で固定した方針）
   - Phase 3 で採用した事前リファクタ候補と PR1/PR2 のリンク（あれば「参考情報」または「関連 PR」として軽く触れる）
   - Phase 5 / Phase 6 で残った指摘・スキップ理由
   - `git log {ベースブランチ}..HEAD --oneline` / `git diff {ベースブランチ}...HEAD --stat`
4. description を組み立てるときの構造:
   - 冒頭は **背景 / 概要に相当する見出し**（テンプレートに該当見出しがあればそれ、無ければ `## 背景`）の直下に `-` 箇条書き **3 行**:
     - 1 行目: `- Linear: {linear_issue.url}`
     - 2-3 行目: 「なぜこの変更が必要か」「何を解決するか」を 1 行ずつ
   - **判断と選択肢**: `design_summary` で比較した代替案・採用理由・トレードオフを `<details><summary>判断と選択肢</summary>...</details>` で畳んで残す。検討が無ければ書かない
   - **変更内容（要点のみ）**: 2-4 行の見取り図のみ。ファイル列挙・行数・逐次手順は書かない
   - **スコープ外** / **後続タスク** / **参考情報**: 該当があるときだけ追加（空セクションを機械的に作らない）
   - Phase 6 で HIGH severity が残存している場合は、目立つ位置に注意喚起を 1 行入れる（例: `> ⚠️ Security HIGH 残存: {内容}（別途対応予定）`）

### Step 3: draft PR を作成

1. 生成した description を一時ファイル（例: `/tmp/feature_pr_body.md`）に書き出す
2. `gh pr create --draft --base {ベースブランチ} --head {feature_branch_name} --title "{タイトル}" --body-file /tmp/feature_pr_body.md` を実行する
   - タイトル: Linear identifier を先頭に置き、簡潔に（例: `{linear_issue.identifier}: {要点}`）。テンプレートやリポジトリ慣習があればそれに従う
3. レスポンスから PR URL を取得し、`feature_pr_url` に格納する

### Step 4: セルフレビューと description 修正

`claude/commands/pr.md` のセルフレビュー観点（冒頭 3 行箇条書きの形式、テンプレート見出しの保持、`<details>` での畳み、HOW の冗長列挙混入、空セクション、機密情報、絵文字・定型句、Markdown 崩れ・誤字）でレビューする。

1. `gh pr view {PR番号} --json body --jq .body` で実際にレンダリングされる本文を取得して確認
2. 修正が必要な場合は本文を編集して `gh pr edit {PR番号} --body-file /tmp/feature_pr_body.md` で上書き
3. 修正不要と判断した場合でも、「セルフレビュー済み・修正不要」と次フェーズの報告に含める（黙って終わらせない）

### Step 5: Linear issue へのリンク

`mcp__claude_ai_Linear__create_attachment` で `feature_pr_url` を `linear_issue.id` に紐付ける（タイトルは PR タイトルと同じか、`Draft PR: {タイトル}` 等）。失敗した場合はユーザーに手動リンクを促す。

### Step 6: Journal Update

`/dev-journal` に以下を記録:

- 作成した PR の URL / タイトル / ベースブランチ
- description に組み込んだ WHY と判断・選択肢の要点
- セルフレビューで修正した箇所（または「修正不要」）
- Linear issue へのリンク結果
- スキップした場合の理由
...
PR: {feature_pr_url}{skip 時: "skipped (reason: ...)"}
