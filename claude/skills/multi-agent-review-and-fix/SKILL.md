...
description: ハーネスエンジニアリングに基づく機能開発伴走スキル。新機能の実装を依頼されたとき、設計から実装まで一気通貫で進めたいとき、要件が曖昧な段階から整理しつつ作り込みたいとき、Linear/GitHub Issue から機能開発を始めるときに使う。Codex主導で要件把握→設計検討→初回実装を行い、その後 Codexレビュー→Claudeトリアージ→修正→検証のループで品質を段階的に高める。「機能Xを実装したい」「設計から実装まで進めて」「新機能の開発を始めたい」「要件を整理しながら作りたい」などのフレーズで起動する。既に存在する差分への多段レビューにも応用可能だが、その場合でも要件と設計の合意を取り直してから進める。PR URL はベースブランチ推定にも使えるが、実際のレビュー対象は現在のローカル HEAD とする。
...
cat << 'PROMPT' | codex exec --ephemeral -m gpt-5.5 -c model_reasoning_effort="xhigh" -s read-only -o /tmp/codex_requirements.txt -
...
cat << 'PROMPT' | codex exec --ephemeral -m gpt-5.5 -c model_reasoning_effort="xhigh" -s read-only -o /tmp/codex_design.txt -
...
cat << 'PROMPT' | codex exec --ephemeral -m gpt-5.5 -c model_reasoning_effort="xhigh" -s read-only -o /tmp/codex_review.txt -
